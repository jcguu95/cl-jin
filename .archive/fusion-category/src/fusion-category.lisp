...
(defgeneric fusion-data? (C data))
(defmethod fusion-data? ((C multifusion-category) data)
  "Test if DATA is a fusion data for the multifusion-category
  C."
  (let ((iof (iof C))
        (rank (rank C))
        (fusion-data (fusion-data C)))

    (and

     ('((a . c)))
     (loop for i from 0 to (- rank 1)
           do (loop for j in from i to j
                    do (loop for k from j to k) (assoc (cons i j) data)
                    ))
